...
%description
This package contains the Infrastructure Data Path Function Linux Driver.
...
echo "override idpf * extra" >> %{buildroot}/etc/depmod.d/idpf.conf

cd %{buildroot}

echo "/usr/src/%{name}-%{version}/*" \
	>>%{_builddir}/%{name}-%{version}/file.list
echo "/etc/depmod.d/idpf.conf" \
	>>%{_builddir}/%{name}-%{version}/file.list
...
# No weak modules, as dkms should take care of kernel updates already
...
# No weak modules, since dkms remove should unlinked weak updates already
