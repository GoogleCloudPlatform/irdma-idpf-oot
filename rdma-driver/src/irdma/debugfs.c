...
static void dump_stats_cmd(struct irdma_device *iwdev)
{
	...
	dbg_vsnprintf("AH deletion list cnt curr(nosleep)  %u\n",
		      iwdev->ah_nosleep_deletion_list_cnt);
	dbg_vsnprintf("AH deletion list cnt total(nosleep) %u\n",
		      iwdev->ah_nosleep_deletion_list_cnt_total);
	dbg_vsnprintf("AH deletion list cnt peak(nosleep)  %u\n",
		      iwdev->ah_nosleep_deletion_list_cnt_peak);
...
static void dump_hw_stats_cmd(struct irdma_device *iwdev)
{
	...
	if (iwdev->rf->rdma_ver >= IRDMA_GEN_3) {
		...
		dbg_vsnprintf("Rx atomics                   %llu\n",
			      hw_stats->stats_val[IRDMA_HW_STAT_INDEX_RDMARXATS]);
		dbg_vsnprintf("Tx atomics                   %llu\n",
				   ...
				   loff_t *ppos)
{
	...
	if (cmddone) {
		...
		else if (strncasecmp(cmd_buf, "cqp-nop", 7) == 0)
