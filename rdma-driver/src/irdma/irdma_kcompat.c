...
/**
 * irdma_add_qs_handle - add new qset handle on PD
 * @iwpd: protection domain
 * @qs_handle: qset handle to be aded
 */
static void irdma_add_qs_handle(struct irdma_pd *iwpd, u16 qs_handle)
{
	int i;

	for (i = 0; i < IRDMA_MAX_QSETS; i++)
		if (iwpd->qs_pages[i].qs_handle == IRDMA_INVALID_QS_HANDLE) {
			iwpd->qs_pages[i].qs_handle = qs_handle;
...
/**
 * irdma_setup_qsets - add qset handles for a user priority
 * @iwpd: protection domain
 * @qos: qset handles for each user priority
 */
static void irdma_setup_qsets(struct irdma_pd *iwpd, struct irdma_qos *qos)
{
	u16 qs_handle;
	int index;
	int i;

	for (i = 0; i < qos->qs_cnt; i++) {
		qs_handle = qos->qs_handle[i];
		index = irdma_find_qs_handle(iwpd, qs_handle);
		if (index >= IRDMA_MAX_QSETS)
			irdma_add_qs_handle(iwpd, qs_handle);
	}
}

/**
 * irdma_init_qs_handles - initialize all qset handles in pd
 * @iwdev: device for vsi
 * @iwpd: protection domain
 */
static void irdma_init_qs_handles(struct irdma_device *iwdev, struct irdma_pd *iwpd)
{
	struct irdma_sc_vsi *vsi = &iwdev->vsi;
	u8 i;

	for (i = 0; i < IRDMA_MAX_USER_PRIORITY; ++i)
		irdma_setup_qsets(iwpd, &vsi->qos[i]);
}

/**
 * irdma_init_qs_pages - initialize all qset pages
 * @qs_pages: all qset pages of the protection domain
 */
static void irdma_init_qs_pages(struct irdma_qs_push_pages *qs_pages)
{
	struct irdma_hw_push_page *hw_page;
	int i;

	qs_pages->qs_handle = IRDMA_INVALID_QS_HANDLE;
	for (i = 0; i < IRDMA_MAX_PUSH_PAGES_QSET; i++) {
		hw_page = &qs_pages->push_page[i];
		bitmap_zero(hw_page->push_offset_bmap, IRDMA_QPS_PER_PUSH_PAGE);
		hw_page->push_idx = IRDMA_INVALID_PUSH_PAGE_INDEX;
	}
}

/**
 * irdma_init_pd_qos - setup all qos for the protection domain
 * @iwdev: device for vsi
 * @iwpd: protection domain
 */
static void irdma_init_pd_qos(struct irdma_device *iwdev, struct irdma_pd *iwpd)
{
	int i;

	for (i = 0; i < IRDMA_MAX_QSETS; i++)
		irdma_init_qs_pages(&iwpd->qs_pages[i]);

	irdma_init_qs_handles(iwdev, iwpd);
}
...
/**
 * irdma_ah_hash_delete - Helper function to decrement refcnt and delete
 *                        hashtable entry/free parent_ah if refcnt goes to 0.
 * @iwdev: irdma device
 * @ah: pointer to irdma address handle
 *
...
/**
 * irdma_create_ah_v2 - create address handle
 * @ib_ah: ptr to AH
 * @attr: address handle attributes
 * @ah_flags: AH flags to wait
 * @udata: user data
...
int irdma_create_ah(struct ib_ah *ib_ah,
		    ...
		    struct ib_udata *udata)
{
	return irdma_create_ah_v2(ib_ah, init_attr->ah_attr, init_attr->flags, udata);
}
#endif /* CREATE_AH_VER_5 */
...
#define IRDMA_CREATE_CQ_MIN_RESP_LEN offsetofend(struct irdma_create_cq_resp, cq_size)
	...
	if (dev->hw_wa & CQ_NO_CHECKFLOW)
		cqp_info->in.u.cq_create.check_overflow = false;
...
int irdma_destroy_cq(struct ib_cq *ib_cq, struct ib_udata *udata)
{
	...
	if (dev->hw_wa & CCQ_CQ3_POLL && rf->cq_id_3 && rf->cq_id_3 == iwcq)
		rf->cq_id_3 = NULL;
...
static const char *const irdma_hw_stat_names[] = {
	...
	[IRDMA_HW_STAT_INDEX_RDMARXATS]         = "Rx atomics",
	[IRDMA_HW_STAT_INDEX_RDMATXATS]         = "Tx atomics",
	...
	/* gen4 */
	[IRDMA_HW_STAT_INDEX_RDMARXFLUSH]       = "Rx Flush",
	[IRDMA_HW_STAT_INDEX_RDMATXFLUSH]       = "Tx Flush",
	[IRDMA_HW_STAT_INDEX_RDMARXATOMICWRITE] = "Rx Atomic Write",
	[IRDMA_HW_STAT_INDEX_RDMATXATOMICWRITE] = "Tx Atomic Write",
};
#ifdef static_assert

static_assert(ARRAY_SIZE(irdma_hw_stat_names) >= IRDMA_HW_STAT_INDEX_MAX_GEN_4,
	      "irdma_hw_stat_names and max_stat_idx mismatch");
#endif /* static_assert */
