...
static const char *const irdma_cqp_cmd_names[IRDMA_MAX_CQP_OPS] = {
	...
	[IRDMA_OP_GATHER_RDMA_SYSTEM_STATS] = "Gather RDMA System Stats Cmd",
	[IRDMA_OP_NOP] = "No-op CQP OP Cmd",
	[IRDMA_OP_RET_CQP_CMPL] = "Return CQP Completion Cmd",
	[IRDMA_OP_RCA_EXEC_FWD_OP] = "Execute Forwarded CQP OP Cmd",
	[IRDMA_OP_COPY_DATA] = "Copy Data to HMC Func Cmd",
};
...
static const struct irdma_cqp_err_info irdma_noncrit_err_list[] = {
	...
	{0xf002, 0x0001, "RCA Function Reset"},		/* for RCA */
};
...
/**
 * irdma_find_qs_handle - check if qset handle is already in pd
 * @iwpd: protection domain
 * @qs_handle: qset handle
 */
int irdma_find_qs_handle(struct irdma_pd *iwpd, u16 qs_handle)
{
	...
	for (i = 0; i < IRDMA_MAX_QSETS; i++)
		if (iwpd->qs_pages[i].qs_handle == qs_handle)
			break;
...
/**
 * irdma_free_push_page_3 - free a push page for qp
 * @rf: RDMA PCI function
 * @iwqp: QP pointer
 * @hw_page: push page to free
 */
static void irdma_free_push_page_3(struct irdma_pci_f *rf,
				   struct irdma_qp *iwqp,
				   struct irdma_hw_push_page *hw_page)
{
	struct irdma_sc_qp *qp = &iwqp->sc_qp;
	bool is_empty;
	u32 push_pos;
	...
	__clear_bit(push_pos, hw_page->push_offset_bmap);
	is_empty = bitmap_empty(hw_page->push_offset_bmap, IRDMA_QPS_PER_PUSH_PAGE);
	if (!is_empty) {
...
/**
 * irdma_dealloc_push_page_3 - handle push page for free qp
 * @rf: RDMA PCI function
 * @iwqp: QP pointer
 */
static void irdma_dealloc_push_page_3(struct irdma_pci_f *rf, struct irdma_qp *iwqp)
{
	struct irdma_sc_qp *qp = &iwqp->sc_qp;
	struct irdma_qs_push_pages *qs_pages;
	...
	for (idx = 0; idx < IRDMA_MAX_PUSH_PAGES_QSET; idx++) {
		found = qp->push_idx == qs_pages->push_page[idx].push_idx ? true : false;
		if (found) {
			irdma_free_push_page_3(rf, iwqp, &qs_pages->push_page[idx]);
			break;
		}
	}
exit:
			     ...
			     struct irdma_qp *iwqp)
{
	...
	if (iwqp->iwdev->rf->sc_dev.hw_attrs.uk_attrs.hw_rev >= IRDMA_GEN_3)
		return irdma_dealloc_push_page_3(rf, iwqp);
...
/**
 * irdma_cqp_nop - issue cqp NOP command
 * @dev: hardware control device structure
 **/
int irdma_cqp_nop(struct irdma_sc_dev *dev)
{
	...
	cqp_info = &cqp_request->info;
	cqp_info->cqp_cmd = IRDMA_OP_NOP;
	cqp_info->post_sq = 1;
	cqp_info->in.u.nop.cqp = &rf->cqp.sc_cqp;
	cqp_info->in.u.nop.scratch = (uintptr_t)cqp_request;
	status = irdma_handle_cqp_op(rf, cqp_request);
	irdma_put_cqp_request(&rf->cqp, cqp_request);
...
void irdma_cqp_rca_post_rqes(struct irdma_sc_dev *dev)
{
	...
	/* post all RQ entries to RCA RQ */
	irdma_sc_cqp_post_rqes(&rf->cqp.sc_cqp);
}

/**
 * irdma_cqp_ret_cqp_cmpl - issue cqp command to return RCA completion
 * @dev: hardware control device structure
 * @info: info for RCA completion
 **/
			   ...
			   struct irdma_ret_cqp_cmpl_info *info)
{
	...
	cqp_info = &cqp_request->info;
	memcpy(&cqp_info->in.u.ret_cqp_cmpl.info, info,
	       sizeof(cqp_info->in.u.ret_cqp_cmpl.info));
	cqp_info->cqp_cmd = IRDMA_OP_RET_CQP_CMPL;
	cqp_info->post_sq = 1;
	cqp_info->in.u.ret_cqp_cmpl.cqp = &rf->cqp.sc_cqp;
	cqp_info->in.u.ret_cqp_cmpl.scratch = (uintptr_t)cqp_request;
	status = irdma_handle_cqp_op(rf, cqp_request);
	irdma_put_cqp_request(&rf->cqp, cqp_request);
...
/**
 * irdma_cqp_ret_cqp_cmpl_data - populate ret_cmpl_info structure
 * @fwd_op_info: fwd op info
 * @compl_info: info for fwd op completion
 * @ret_cmpl_info: structure to be populated
 **/
static void irdma_cqp_ret_cqp_cmpl_data(
	const struct irdma_rca_exec_fwd_op_info *fwd_op_info,
	...
	struct irdma_ret_cqp_cmpl_info *ret_cmpl_info)
{
	...
	ret_cmpl_info->cqe_values[2] =
		...
		FIELD_PREP(IRDMA_CCQ_OPRETVAL, compl_info->op_ret_val);

	ret_cmpl_info->cqe_values[3] =
		FIELD_PREP(IRDMA_CQ_SQ, 1) |
		FIELD_PREP(IRDMACQ_OP, fwd_op_info->op_code) |
		FIELD_PREP(IRDMA_OOO_CMPL, fwd_op_info->pending) |
		FIELD_PREP(IRDMA_CQ_WQEIDX, fwd_op_info->orig_wq_desc_idx) |
		FIELD_PREP(IRDMA_CQ_ERROR, compl_info->error) |
		FIELD_PREP(IRDMA_CQ_MAJERR, compl_info->maj_err_code) |
		FIELD_PREP(IRDMA_CQ_MINERR, compl_info->min_err_code);
}

/**
 * irdma_cqp_copy_data_cb - copy data to buffer owned by originating function
 * @cqp_request: CQP request info
 **/
static void irdma_cqp_copy_data_cb(struct irdma_cqp_request *cqp_request)
{
	struct irdma_ret_cqp_cmpl_info ret_cmpl_info = {};
	struct irdma_rca_exec_fwd_op_info *fwd_op_info;
	struct irdma_copy_data_info *copy_data_info;
	struct irdma_cqp_compl_info *compl_info;
	struct cqp_cmds_info *cqp_info;

	cqp_info = &cqp_request->info;
	compl_info = &cqp_request->compl_info;
	copy_data_info = &cqp_info->in.u.copy_data.info;
	fwd_op_info = &copy_data_info->fwd_op_info;

	/* NOTE: The opcode in completion info will be overwritten with
	 * the original opcode of the forwarded CQP op, so it will look like
	 * completion from UPLOAD_CTX, etc.
	 * The operation return value is not relevant.
	 */
	irdma_cqp_ret_cqp_cmpl_data(fwd_op_info, compl_info, &ret_cmpl_info);
	irdma_cqp_ret_cqp_cmpl(cqp_info->in.u.copy_data.cqp->dev,
			       &ret_cmpl_info);
}

/**
 * irdma_cqp_copy_data - copy data to HMC function
 * @dev: hardware control device structure
 * @info: copy data info
 **/
static int irdma_cqp_copy_data(struct irdma_sc_dev *dev,
			       struct irdma_copy_data_info *info)
{
	...
	cqp_info = &cqp_request->info;
	memcpy(&cqp_info->in.u.copy_data.info, info,
	       sizeof(cqp_info->in.u.copy_data.info));
	cqp_info->cqp_cmd = IRDMA_OP_COPY_DATA;
	cqp_info->post_sq = 1;
	cqp_info->in.u.copy_data.cqp = &rf->cqp.sc_cqp;
	cqp_info->in.u.copy_data.scratch = (uintptr_t)cqp_request;

	cqp_request->callback_fcn = irdma_cqp_copy_data_cb;
...
/**
 * irdma_cqp_rca_cb - return completion to originating function
 * @cqp_request: CQP request info
 **/
static void irdma_cqp_rca_cb(struct irdma_cqp_request *cqp_request)
{
	struct irdma_ret_cqp_cmpl_info ret_cmpl_info = {};
	struct irdma_copy_data_info copy_data_info = {};
	struct irdma_rca_exec_fwd_op_info *fwd_op_info;
	 ...
	 * Otherwise, save the completion data, issue COPY_DATA
	 * and once it is completed, return the original completion
	 * to originating function.
	 */
	if (fwd_op_info->op_code == IRDMA_CQP_OP_UPLOAD_CONTEXT &&
	    !compl_info->error) {
		memcpy(&copy_data_info.fwd_op_info, fwd_op_info,
		       sizeof(*fwd_op_info));

		copy_data_info.src_addr = fwd_op_info->buf_addr;
		copy_data_info.dst_addr = fwd_op_info->wqe[2];
...
/**
 * irdma_cqp_rca_exec_fwd_op - execute forwarded CQP op on behalf
 *   of originating function
 * @dev: hardware control device structure
			      ...
			      struct irdma_rca_exec_fwd_op_info *info)
{
	...
	cqp_info->cqp_cmd = IRDMA_OP_RCA_EXEC_FWD_OP;
	cqp_info->post_sq = 1;
			   ...
			   void *cb_param)
{
	...
	} else if (cmd == IRDMA_OP_AH_MODIFY) {
		cqp_info->in.u.ah_modify.info = sc_ah->ah_info;
		cqp_info->in.u.ah_modify.scratch = (uintptr_t)cqp_request;
		cqp_info->in.u.ah_modify.cqp = &rf->cqp.sc_cqp;
		cqp_info->create = true;
	}
...
{
	return ioremap(pci_resource_start(dev_to_rf(dev)->pcidev, 0), size);
}
