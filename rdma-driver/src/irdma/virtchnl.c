				      ...
				      u16 len)
{
	...
	case IRDMA_VCHNL_OP_ADD_VPORT:
		op_size = sizeof(struct irdma_vchnl_req_vport_info);
		if (len < sizeof(*vchnl_msg) + op_size)
			return false;
		break;
				       ...
				       u16 resp_len)
{
	...
	case IRDMA_VCHNL_OP_GET_MULTI_QS:
	case IRDMA_VCHNL_OP_GET_VER:
	case IRDMA_VCHNL_OP_GET_HMC_FCN:
	case IRDMA_VCHNL_OP_PUT_HMC_FCN:
	case IRDMA_VCHNL_OP_ADD_HMC_OBJ_RANGE:
...
{
	struct irdma_vchnl_resp_vport_info resp_vport = {};
	struct irdma_vchnl_req_vport_info req_vport = {};
	struct irdma_vchnl_req_init_info info = {};
	int ret, i;
	...
	for (i = 0;  i < IRDMA_MAX_USER_PRIORITY; i++) {
		qos[i].qs_handle[0] = resp_vport.qs_handle[i];
		qos[i].valid = true;
	}
...
/**
 * irdma_vchnl_req_get_multi_qs - Request VF qsets
 * @dev: RDMA device pointer
 */
int irdma_vchnl_req_get_multi_qs(struct irdma_sc_dev *dev)
{
	struct irdma_vchnl_req_multi_qs_info req_multi_qs = {};
	struct irdma_vchnl_resp_multi_qs_info resp_multi_qs = {};
	struct irdma_vchnl_req_init_info info = {};
	struct irdma_qs_entry *entry;
	int ret, i;

	if (!dev->vchnl_up)
		return -EBUSY;

	req_multi_qs.vport_id = 0;
	info.op_code = IRDMA_VCHNL_OP_GET_MULTI_QS;
	info.req_parm_len = sizeof(req_multi_qs);
	info.req_parm = &req_multi_qs;
	info.resp_parm = &resp_multi_qs;
	info.resp_parm_len = sizeof(resp_multi_qs);
	...
	dev->qos_dist_mode = resp_multi_qs.dist_mode;
	for (i = 0;  i < IRDMA_MAX_USER_PRIORITY; i++) {
		entry = &resp_multi_qs.qs_entry[i];
		dev->qos[i].qs_cnt = entry->qs_cnt;
		memcpy(dev->qos[i].qs_handle,
			     entry->qs_handle,
			     entry->qs_cnt * sizeof(u16));
	}
...
int irdma_vchnl_req_get_caps(struct irdma_sc_dev *dev)
{
	...
	info.op_ver = IRDMA_VCHNL_OP_GET_RDMA_CAPS_V1;
